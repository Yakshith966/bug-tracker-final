<template>
  ...
  export default {
    ...
    methods: {
     updateBugStatus(bug, newStatus) {
      ...
      axios
        .post(`http://127.0.0.1:8000/api/update-task-status/${bug.id}`, { status: selectedStatus }, {
          headers: {
        ...
        .then((response) => {
          ...
          this.fetchBugs();
          this.showSnackbar('Task status updated successfully', 'success');
        })
        .catch((error) => {
          console.error('Error updating bug status:', error);
          this.showSnackbar('Failed to update Task status', 'error');
        });
    },
  ...
  try {
    ...
    const taskImages = item.task_images ? JSON.parse(item.task_images) : [];
...
viewBugDetails(bugId) {
        ...
        .then(response => {
          ...
          const taskImages = bug.task_images ? JSON.parse(bug.task_images) : [];

this.bugDetails = {
  title: bug.name,
  description: bug.description,
  status: bug.status,
  assignedDeveloper: bug.developer ? bug.developer.name : null,
  ...
  images: taskImages.map(image => `http://127.0.0.1:8000/storage/${image}`),
      ...
      fetchBugs() {
        ...
        axios
          .get(`http://127.0.0.1:8000/api/tasks/tester/${this.developerId}`, {
            headers: {
              Authorization: `Bearer ${sessionStorage.getItem("token")}`,
            },
          })
